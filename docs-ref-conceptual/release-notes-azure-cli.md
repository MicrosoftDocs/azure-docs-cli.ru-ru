...
* Включена поддержка PPG.
* Изменено максимальное время ожидания для подсистемы балансировки нагрузки уровня "Стандартный" на значение в 100 минут.

### <a name="apim"></a>APIM

* Исправлена проблема с созданием экземпляра уровня "Потребление".

### <a name="app-config"></a>Конфигурация приложения

* Исправлен процесс запрашивания пар "ключ — значение" по меткам с разделителями-запятыми.

### <a name="app-service"></a>Служба приложений

* Исправлена проблема, при которой команда az webapp up не выполнялась, если у пользователя не было разрешений на запись в родительский каталог проекта.
* Исправление 13777: исправлена проблема с удалением escape-символов из XML.
* Исправление 15441: исправлена проблема, при которой выполнение az webapp create-remote-connection завершалось сбоем с ошибкой "AttributeError: 'Thread' object has no attribute 'isAlive'" (У объекта Thread нет атрибута isAlive).
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ.] az webapp up: добавлены необязательные параметры (среда выполнения и ОС) и обновлены среды выполнения.

### <a name="arm"></a>ARM

* Команды What-If развертывания шаблона сделаны общедоступными.
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ.] Добавлено подтверждение пользователя для az ts create.
* Исправлена проблема с возвращаемыми данными при создании тегов для нескольких ресурсов.

### <a name="backup"></a>Резервное копирование

* `az backup policy create`: включена поддержка создания политики резервного копирования IaaSVM из интерфейса командной строки.
* Увеличено ограничение защиты виртуальных машин со 100 до 1000.

### <a name="compute"></a>Службы вычислений

* sig image-definition create: добавлен параметр --features.
* Новая версия API gallery_images 2020-09-30.
* `az vm update / az sig image-version update`: включено обновление виртуальной машины или версии ее образа, даже если в ней используется межклиентский образ.
* Отключена проверка номеров SKU узла виртуальной машины.

### <a name="cosmos-db"></a>Cosmos DB

* `az cosmosdb create/update`: улучшено сообщение об ошибке из-за неправильного ввода --locations.
* `az cosmosdb sql container create/update`: добавлен параметр --analytical-storage-ttl.

### <a name="hdinsight"></a>HDInsight

* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ.] az hdinsight create: удалены два параметра — --public-network-access-type и --outbound-public-network-access-type.

### <a name="iot-central"></a>IoT Central

* Удалено предупреждение о предварительной версии, так как уже предоставляется общедоступная версия.

### <a name="key-vault"></a>Key Vault

* `--enable-soft-delete false` больше не используется при создании или обновлении хранилищ.
* `--bypass` и `--default-action` используются совместно с параметрами ACL сети при создании хранилищ.

### <a name="misc"></a>Прочее

* Добавлена коллекция bash-completion в Dockerfile.

### <a name="rdbms"></a>Реляционная СУБД

* Добавлена команда list-skus, преобразователь таблиц, локальный контекст для Postgres, MySQL, отдельный сервер MariaDB.
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ.] Обновлено имя параметра. Улучшения в плоскости управления для MySQL и PostgreSQL.
* `az postgres|mariadb|mysql server create`: обновлен интерфейс создания для Postgres, MySQL и MariaDB — новые поля в выходных данных; представлены новые значения для параметра `--public` в команде CREATE (all,<IP>,<IPRange>,0.0.0.0).

### <a name="signalr"></a>SignalR

* `az signalr create`: добавлен новый параметр `--enable-messaging-logs` для управления процессом создания службой журналов сообщений.
* `az signalr update`: добавлен новый параметр `--enable-messaging-logs` для управления процессом создания службой журналов сообщений.

### <a name="sql"></a>SQL

* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ.] Исправлен ответ для имени и значения параметра избыточности хранилища резервных копий для MI.
* `az sql db audit-policy show`: добавлено расширение для отображения политики аудита базы данных, включая данные о LA и EH.
* `az sql db audit-policy update`: добавлено расширение, разрешающее выполнять обновление LA и EH вместе с политикой аудита базы данных.
* `az sql db audit-policy wait`: переключение интерфейса командной строки в состояние ожидания, пока не будет выполнено условие политики аудита базы данных.
* `az sql server audit-policy show`: добавлено расширение для отображения политики аудита сервера, включая данные о LA и EH.
* `az sql server audit-policy update`: добавлено расширение, разрешающее выполнять обновление LA и EH вместе с политикой аудита сервера.
* `az sql server audit-policy wait`: переключение интерфейса командной строки в состояние ожидания, пока не будет выполнено условие политики аудита сервера.
* Включена поддержка только AAD для Управляемых экземпляров и серверов SQL.
* `az sql db replica create`: добавлен аргумент --partner-database.

### <a name="storage"></a>Память

* Исправление 15111: исправлена проблема, при которой выполнение `az storage logging update` завершалось сбоем без дополнительного аргумента.
* Устранена проблема с использованием команды set-tier с именем входа субъекта-службы.
* Обновление версии file-datalake до 2020-02-10
* `az storage queue list`: включена поддержка Track2.
* `az storage fs access`: включена поддержка рекурсивного управления списками ACL.

### <a name="synapse"></a>Synapse

* Добавлены командлеты, связанные с конвейером, связанной службой, триггером, записной книжкой, потоком данных и набором данных.
