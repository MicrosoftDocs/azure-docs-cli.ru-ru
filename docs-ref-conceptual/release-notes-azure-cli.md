...
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ] Параметр `capabilities` в команде `Catalog` переименован в `sku_properties`.
...
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ]. Удален параметр `--ids` для:
...
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ]. Удалено `--msi-port` и `--identity-port`, так как они больше не используются.
...
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ]. Удалено `ad sp reset-credentials` по причине устаревания.
...
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ]. Не используйте ManagedIdentityExtension виртуальной машины для включения поддержки MSI.
* Добавлена поддержка политики вытеснения для `vmss`.
* [КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ]. Удалено `--ids` из:
  * `vm extension list`
  * `vm secret list`
