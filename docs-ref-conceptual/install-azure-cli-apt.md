...
* Ubuntu Trusty, Xenial, Bionic, Eoan и Focal;
* Debian Jessie, Stretch и Buster.
...
>
> Для Ubuntu 20.04 (Focal) доступен пакет `azure-cli` с версией `2.0.81`, предоставляемый репозиторием `focal/universe`. Он устарел и не рекомендуется к использованию. Если вы уже установили его, сначала удалите этот пакет с помощью команды `sudo apt remove azure-cli -y && sudo apt autoremove -y`, прежде чем выполнять приведенные ниже шаги для установки последней версии пакета `azure-cli`.
